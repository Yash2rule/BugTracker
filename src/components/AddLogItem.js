...
const AddLogItem = ({addItem}) => {
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('error:info',(e, error) => {
            ...
            setError(parsedError.response && parsedError.response.data.message
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        if(text === '' || description === '' || platform === '' || priority === '' || image === ''){
        ...
        <Card className='mt-5 mb-3' style={{border: '1px solid grey'}}>
            ...
            {alert.show && <Alert variant={alert.variant}>{alert.message}</Alert>}
                <Form onSubmit={onSubmit}>
                    <Row className='my-3'>
                        <Col>
                            <Form.Control placeholder='Bug Title' value={text} onChange={(e) => setText(e.target.value)}/>
                        </Col>
                    </Row>
                    <Row className='my-3'>
                        <Col>
                            <Form.Control placeholder='Bug Description' value={description} onChange={(e) => setDescription(e.target.value)}/>
                        </Col>
                    </Row>
                    <Row className='my-3'>
                        <Col>
                        <Form.Group controlId='image'>
                            <Form.Control type='text' placeholder='Image Location'
                    ...
                    <Row className='my-3'>
                        <Col>
                            <Button type='submit' variant='secondary' className='btn btn-dark' block>
                                Add Log
                            </Button>
                        </Col>
                    </Row>
