...
const App = () => {
	...
	<Route path={`/logs/:id/edit`} render={({history}) => <LogEditScreen history={history}/>} exact/>
	<Route path={`/logs/:id/solution`} render={({history}) => <LogSolution history={history}/>} exact/>
	</>
	)
}
