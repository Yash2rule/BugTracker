...
const LogsScreen = () => {
	...
	useEffect(() => {
			...
			ipcRenderer.on('logs:get',(e,logs) => {
				const sortedLogs = JSON.parse(logs).sort((a, b) => new Date(b.created.substring(0,19)) - new Date(a.created.substring(0,19)));
				if(isMounted){
					setLogs(sortedLogs);
				}
			})
	...
	function addItem(item) {
		if(item.text === '' || item.platform === '' || item.priority === '' || item.description === ''){
	...
	return (
		<Container>
			<LinkContainer to={`/`}>
                <Button variant='dark' className='btn-md my-3' onClick={userInfo ? () => localStorage.removeItem('userInfo') : null} >
                   {userInfo && 'Logout'}
                </Button>
            </LinkContainer>
			...
			{logs.length > 0 && (
				<Dropdown>
				<Dropdown.Toggle variant="success" id="dropdown-basic" drop='none' className='my-3'>
					<i className="fa fa-sort"></i>{' '} Sort By
				</Dropdown.Toggle>
				<Dropdown.Menu>
					<Dropdown.Item as="span" style={{color:'black'}} onClick={sortByPriority}>Priority</Dropdown.Item>
					{/* <Dropdown.Item as="span" style={{color:'black'}} onClick={sortByDateAndTime}>Date</Dropdown.Item> */}
				</Dropdown.Menu>
				</Dropdown>
			)}
			
			{logs.length === 0 ? <h1 style={{textAlign:'center',marginTop:'40px'}}>No Logs Posted</h1> : (
